...
CREATE TABLE public.backup_history (
  ...
  tables_included TEXT[], record_count INT, created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
ALTER TABLE public.backup_history ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can read own backups" ON public.backup_history FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Users can insert backups" ON public.backup_history FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);

CREATE TABLE public.app_settings (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  key TEXT NOT NULL UNIQUE, value TEXT, updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
